...
import CSS from "./Punjab.module.css";
...
class Punjab extends React.Component {
  ...
  handleChange = e => {
    ...
    if (
      (splitNum[0] &&
        splitNum[1] &&
        splitNum[0].length === 3 &&
        (splitNum[1].length === 3 || splitNum[1].length === 4) &&
        /^[0-9]*$/.test(splitNum[1]) &&
        /^[A-Z]+$/.test(splitNum[0])) ||
      (splitNum[0] &&
        splitNum[1] &&
  ...
  submit = () => {
    axios
      .post("https://vehicleinfo.azurewebsites.net//Punjab", {
        vhlno: this.state.number
      })
      .then(res =>
    ...
    return (
      <div className={[CSS.container, "form-group"].join(" ")}>
        ...
        <button
          ...
          onClick={this.submit}
        >
          {this.state.invalid ? "AAA 111 / AAA 1111 / AAA-11-1111" : "Search"}
        </button>
      </div>
    );
  }
...
export default withRouter(Punjab);
